...
function agregarDetalle(operacion){
    $.ajax({
        ...
        data:{
            pedido_nro:$("#pedido_nro").val(),
            items_cod:$("#items_cod").val(),
            pdet_precio:$("#items_precio").val(),
...
function listarDetalles() {
    $.ajax({
        ...
        var lineas = "";
        for (rs of resultadoDet) {
            lineas += "<tr onclick='seleccionDetalles(" + JSON.stringify(rs) + ")'>";
                lineas += "<td>";
...
function seleccionDetalles(json){
    Object.keys(json).forEach(function(key) {
        $("#"+key).val(json[key]);
    });
    $(".form-line").attr("class","form-line focused");
...
function getProducto(){
    $.ajax({
        ...
        $.each(lista, function(i, item){
            fila += "<li class='list-group-item' onclick='seleccionProducto("+JSON.stringify(item)+")'>"+item.items_desc+"</li>";
        });
