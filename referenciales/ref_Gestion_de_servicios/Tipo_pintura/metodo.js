...
function confirmarOperacion(){
    ...
    var pregunta = "¿Desea grabar el nuevo tipo de pintura?";
    var titulo = "Alta!";

    if(operacion == 2){
        pregunta = "¿Desea modificar el tipo de pintura seleccionado?";
        titulo = "Modificacion!!!";
    }

    if(operacion == 3){
        pregunta = "¿Desea eliminar el tipo de pintura seleccionado?";
        titulo = "Eliminar!!!";
    }
...
function grabar(){
    $.ajax({
        ...
        url:"controlador.php",
        data:{
            tipo_pintura_cod:$("#tipo_pintura_cod").val(),
            tipo_pintura_descr:$("#tipo_pintura_descr").val(),
        ...
        function(){
            if(respuesta.tipo=="success"){
                $("#tipo_pintura_cod").val(respuesta.ultcod);
                location.reload(true);
            }
        });
    }).fail(function(a,b,c){
    ...
    $.ajax({
        ...
        for(rs of resultado){
            lineas += "<tr onclick='seleccion("+JSON.stringify(rs)+")'>"
                lineas += "<td>";
                    lineas += rs.tipo_pintura_cod;
                lineas += "</td>";
                lineas += "<td>";
                    lineas += rs.tipo_pintura_descr;
                lineas += "</td>";
                lineas += "<td>";
            lineas += "</tr>"
        }
